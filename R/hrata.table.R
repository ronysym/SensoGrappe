...
hrata.table<-function(res.agreg,select.judge=FALSE,type="dravnieks")
    {
      ...
      if (type=="mean"){ # a modifier sur la base de latt
        ...
        for (i in 1:length(latt)){
          for (k in 1:nlevels(data$Produit)){
            data.table[k,i]<-mean(latt[[i]][which(latt[[i]][,2]==levels(data$Produit)[k]),][,3])
          }
        }
      ...
      if (type=="dravnieks"){
          ...
          for (i in 1:length(latt)){
            for (k in 1:nlevels(data$Produit)){
              ...
              pik<-length(which(latt[[i]][latt[[i]][,2]==levels(data$Produit)[k],3]>0)) # nombre de sujets ayant donn? une note positive ? l'attribut i pour le produit k
              pk<-length(latt[[i]][latt[[i]][,2]==levels(data$Produit)[k],3]) # nombre de sujets ayant ?valu? l'attribut i pour au moins 1 produit
              xi<-sum(latt[[i]][latt[[i]][,2]==levels(data$Produit)[k],3]) # somme des notes de l'attribut i pour le produit k
