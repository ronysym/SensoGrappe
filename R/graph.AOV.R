...
graph.AOV <- function(AOV.res=NULL, factor=NULL, attribute=NULL, horizontal=FALSE, pvalue = TRUE,title.graph=NULL, ymin=0, ymax=NULL, angle.x=0, angle.y=0, angle.grp=0, size.x=10,size.grp=4,save.graph=FALSE, x.ordered=3, prefix=NULL, suffix=NULL, add.moy=FALSE, extension="wmf", color.graph=NULL,y.label="Mean",x.label=NULL)
{
  ...
  if(angle.x > 0)   { hx <- 1 }
  if(angle.x < 0)   { hx <- 0 }
  if(angle.x == 90) { vx <- 0.5 } else { vx <- 1 }
  if(angle.y != 0)  { vy <- 0.5 }
  if(angle.y == 90) { hy <- 0.5 } else { hy <- 1 }
  if(angle.grp != 0)  { hgrp <- 0 }
  ...
  if(is.null(factor))
  {
    ...
    for(fv in 1:length(factor))
    {
      ...
      if(verif == 0) { stop("Erreur. Verifiez l'orthographe des facteurs") }
    }
  }
  ...
  for(f in 1:length(factor))
  {
    ...
    } else
    {
      ...
      for(a in 1:length(attribute))
      {
        ...
        if(sum(verif) == 0) { stop("Erreur : Verifiez l'orthographe des attributs") }
      }
    ...
    for(i in block)
    {
      ...
      if(pvalue.temp < 0.0001) {pvalue.temp="< 0.0001"}
      ...
      if(is.null(title.graph)==TRUE) {name.temp <- colnames(data.temp)[1]}
      else {name.temp <-title.graph}
      ...
      if (is.null(color.graph)==TRUE) { color.graph = c(brewer.pal(12,"Set3"),brewer.pal(8,"Set1"))}
      ...
      {    ymx <- ymax
        if(is.null(ymx)){ ymx <- ceiling((max(data.temp$Moy_num) + max(data.temp$SE_num)) * 1.1) }
      ...
      if(save.graph == TRUE)
      {
        ...
        if(!is.null(prefix)) { nom <- paste(prefix, "_", nom, sep="") }
        if(!is.null(suffix)) { nom <- paste(nom, "_", suffix, sep="") }
      ...
      if(sum(grep(":", factor[f], fixed=TRUE)) != 0)
      {
        ...
        {	#Dessine le graphique des interactions
          ...
          #Chartre graphique
          plot.inter <- plot.inter + theme(panel.grid.major=element_blank(),
                                           panel.grid.minor=element_blank(),
                                           axis.text=element_text(size=12),
                                           axis.text.x=element_text(angle=angle.x, hjust=hx, vjust=vx, size=size.x),
          ...
          #Retourne le graphique a l'horizontal si demande
          if(horizontal == TRUE) { plot.inter <- plot.inter + coord_flip() }
          print(plot.inter)
