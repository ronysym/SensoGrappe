...
#'
#' @export
Var.Grappe <- function(x, column=0, type=NULL, filter=NULL, verbose=TRUE)
{ #Cree la matrice de resultats
  ...
  if (column[1] != 0 && !is.null(type))
  {
    for(i in 1:length(column))
    {
      ...
      if(length(grep(type, fac, ignore.case=TRUE)) > 0) { x[, column[i]]= as.factor(x[, column[i]]) }
      if(length(grep(type, num, ignore.case=TRUE)) > 0) { x[, column[i]]=as.integer(x[, column[i]]) }
      if(length(grep(type, int, ignore.case=TRUE)) > 0) { x[, column[i]]=as.numeric(x[, column[i]]) }
    }
  }
