...
#' PCA.plot.grappe(res.pca, habillage = 13, cex = 0.8)
#'
#' @export
#'
PCA.plot.grappe <- function (x, axes = c(1, 2), choix = c("ind","var","varcor"),
  ...
  ggoptions_default <- list(size = 4, point.shape = 19, line.lty = 2, line.lwd = 0.5, line.color = "grey", segment.lty = 1, segment.lwd = 0.5, circle.lty = 1, circle.lwd = 0.5, circle.color = "grey", low.col.quanti = "blue", high.col.quanti = "red3")
    ...
    if (!is.null(coord.var[ which(apply(res.pca$var$cos2[, axes,drop=FALSE],1,sum, na.rm = TRUE) >= lim.cos2.var),])&is.na(test.invisible[1])&(nrow(coord.var)>0)){
      ...
      if(graph.type == "ggplot"){
        ...
        for (v in 1:nrow(coord.var)) {
          segments(0, 0, coord.var[v, 1], coord.var[v, 2], length = 0.1, angle = 15, code = 2, col = coll[v])
          if (lab.var) {
    ...
    if (graph.type == "classic"){
      ...
      if ((!is.null(res.pca$quanti.sup))&(is.na(test.invisible[2]))){
        ...
        if (isTRUE(lab.quanti)){
          ...
          if (habillage[1] == "cos2"){gg_graph <- gg_graph + geom_segment(aes(x=0,y=0,xend=df_quanti.sup[,2], yend=df_quanti.sup[,3], color = res.pca$quanti.sup$cos2[,axes[1]] + res.pca$quanti.sup$cos2[,axes[2]]),lty = 2,alpha = transparency_quanti)
