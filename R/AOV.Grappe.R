...
AOV.Grappe <-
  function(data,
    ...
    mat.base <-
      matrix(data = NA,
             nrow = length(attr(terms(mod.temp), "term.labels")),
             ncol = nbvar)
    colnam <- NULL
    for (n in 1:nbvar) {
      colnam[n] <- colnames(x[column])[n]
    }
    ...
    for (f in 1:length(Fact))
    {
      ...
      Total.result$lsd.result <-
        c(Total.result$lsd.result, list(lsd.res))
    ...
    if (Random == TRUE) {
      for (A in column)
      {
        ...
        Total.result$signif[, ctm] <-
          STAR.Grappe(Total.result$pvalue[, ctm])
        ...
        rf <- rbind(fv, cf)
        Total.result$fvalue[, ctm] <-
          round(rf[order(rownames(rf), decreasing = F),], 3)

        # Nom de lignes
        rownames(Total.result$fvalue) <- sort(rownames(rf))

        res.base.lsmeans.table <- as.data.frame(ls_means(res.base))
        res.base.difflsmeans.table <-
          as.data.frame(difflsmeans(res.base))
        res.base.lsmeans.table$term <-
          as.factor(res.base.lsmeans.table$term)
        res.base.lsmeans.table$levels <-
          as.factor(res.base.lsmeans.table$levels)
        Fact.base <- levels(res.base.lsmeans.table$term)
        res.step.lsmeans.table <- as.data.frame(ls_means(final_fm))
        res.step.difflsmeans.table <-
          as.data.frame(difflsmeans(final_fm))
        res.step.difflsmeans.table$term <-
          as.factor(res.step.difflsmeans.table$term)
        res.step.difflsmeans.table$levels <-
          as.factor(res.step.difflsmeans.table$levels)
        res.step.lsmeans.table$term <-
          as.factor(res.step.lsmeans.table$term)
        res.step.lsmeans.table$levels <-
          as.factor(res.step.lsmeans.table$levels)
        ...
        for (ph in c(1:length(Fact)))
        {
          ...
          verif.signi <-
            as.matrix(Total.result$pvalue)[facteur, nom_var]

          if (verif.signi > 0.05) {
            sou <-
              res.base.difflsmeans.table[res.base.difflsmeans.table$term == facteur,]
            long <-
              length(rownames((res.base.lsmeans.table[res.base.lsmeans.table$term == facteur,])))
            nam <-
              as.character(res.base.lsmeans.table[res.base.lsmeans.table$term == facteur,]$levels)
            sou1 <- as.data.frame(res.base.lsmeans.table)
          } else if (randomodel != TRUE) {
            sou <-
              res.step.difflsmeans.table[res.step.difflsmeans.table$term == facteur,]
            long <-
              length(rownames((res.step.lsmeans.table[res.step.lsmeans.table$term == facteur,])))
            nam <-
              as.character(res.step.lsmeans.table[res.step.lsmeans.table$term == facteur,]$levels)
          ...
          while (length(grep("  ", rownames(sou))) != 0) {
            rownames(sou) <- gsub("  ", " ", rownames(sou))
          }
          ...
          mat <-
            matrix(
              ...
              ncol = 5,
              nrow = long,
              dimnames = list(1:long, c(
                facteur, "Grp", nom_var, "Std_err", "TMP"
              ))
            )
          ...
          while (v <= (length(mat[, 2]) - 1)) {
            ...
            if ((pmatch(lettre2, Lettre, nomatch = 0) - pmatch(lettre1, Lettre, nomatch = 0)) > 1) {
              ...
              while (lv < length(mat[, 2])) {
                ...
                mat[lv + 1, 2] <-
                  Lettre[pmatch(mat[lv + 1, 2], Lettre) - 1]
                lv <- lv + 1
              }
          ...
          for (l in 1:length(sou$n1))
          {
            ...
            if (sou$"p-value"[l] > 0.05) {
              ...
              # Extrait les lettres
              lettre1 <-
                unlist(strsplit(x = mat[ligne1, 2], split = ""))
              lettre2 <-
                unlist(strsplit(x = mat[ligne2, 2], split = ""))
              ...
              if (verif == 0) {
                ...
                if (pmatch(tail(lettre1, 1), Lettre, nomatch = 0) < pmatch(tail(lettre2, 1), Lettre, nomatch = 0)) {
                  ...
                  new_name <-
                    Lettre[pmatch(lettre1[1], Lettre, nomatch = 0):pmatch(lettre2[1], Lettre, nomatch = 0)]
                  nName <- NULL
                  for (nn in 1:length(new_name)) {
                    nName <- paste(nName, new_name[nn], sep = "")
                  }
                  mat[ligne1, 2] <- nName
                } else {
                  # Si Modalite 1 > Moldalite 2
                  if (pmatch(tail(lettre1, 1), Lettre, nomatch = 0) > pmatch(tail(lettre2, 1), Lettre, nomatch = 0)) {
                    ...
                    new_name <-
                      Lettre[pmatch(lettre2[1], Lettre, nomatch = 0):pmatch(lettre1[1], Lettre, nomatch = 0)]
                    nName <- NULL
                    for (nn in 1:length(new_name)) {
                      nName <- paste(nName, new_name[nn], sep = "")
                    }
          ...
          {
            lol_lettre <-
              head(unlist(strsplit(
                x = (mat[lol, 2]), split = ""
              )), 1)
            lol_positi <- pmatch(lol_lettre, Lettre, nomatch = 1)
            lol_nn <- rep(" ", lol_positi - 1)
            lol_NewNom <- NULL
            for (ll in 0:length(lol_nn)) {
              lol_NewNom <- paste(lol_NewNom, lol_nn[ll], sep = "")
            }
            mat[lol, 2] <- paste(lol_NewNom, mat[lol, 2], sep = "")
          }
          # Arrondi les valeurs
          mat[, c(3, 4)] <-
            format(round(as.numeric(mat[, c(3, 4)]), 2), nsmall = 2)
          grp <- gsub(pattern = " ",
                      replacement = "",
                      x = mat[, 2])
          # Enregistre les resultats
          Total.result$lsd.result[[facteur]] <-
            cbind(Total.result$lsd.result[[facteur]], mat[, c(3, 4, 2)])
        }
        if (verbose == TRUE) {
          cat("\n")
        }
      ...
      for (A in column)
      {
        ...
        fv <- summary(anova)[[1]]["F value"]
        fv <- subset(fv, rownames(fv) != "Residuals     ")
        id.res<-str_detect(rownames(fv), "Residuals")
        fv=cbind(fv,id.res)
        fv <- subset(fv, id.res != TRUE)
        fv <- fv[1]



        Total.result$fvalue[, ctm] <-
          round(fv[order(rownames(fv), decreasing = F),], 3)
        rownames(Total.result$fvalue) <- sort(rownames(fv))
        ...
        for (ph in c(1:length(Fact)))
        {
          ...
          verif.inter <-
            length(unlist(strsplit(
              Fact[ph], split = ":", fixed = TRUE
            )))

          if (verif.inter == 1) {
            lsd.out <- LSD.test(anova, facteur)
            long = length(rownames(lsd.out$groups))
          } else{
            lsd.out <-
              LSD.test(anova, unlist(strsplit(
                Fact[ph], split = ":", fixed = TRUE
              )))
            long = length(rownames(lsd.out$groups))
          }
          ...
          mat <-
            matrix(
              ...
              nrow = long,
              dimnames = list(1:long, c(
                facteur,  nom_var, "Std_err", "Grp"
              ))
            )
          # Ajoute les noms en colonne 1
          mat[, 1] <- rownames(lsd.out$groups)
          rownames(mat)<-rownames(lsd.out$groups)
          # Groups
          mat[, 4] <- lsd.out$groups[[2]]

          # Estimate
          mat[, 2] <- round(lsd.out$groups[[1]], 2)
          # Standard Error
          mat[, 3] <- round(lsd.out$means[[4]], 2)

          mat <- mat[order(mat[, facteur], decreasing = FALSE),]

          # Arrondi les valeurs
          mat[, c(2, 3)] <-
            format(round(as.numeric(mat[, c(2, 3)]), 2), nsmall = 2)
          grp <- gsub(pattern = " ",
                      replacement = "",
                      x = mat[, 2])
          # Enregistre les resultats
          ...
          Total.result$lsd.result[[facteur]] <-
            cbind(Total.result$lsd.result[[facteur]], mat[, c(2, 3, 4)])
        }
        if (verbose == TRUE) {
          cat("\n")
        }
